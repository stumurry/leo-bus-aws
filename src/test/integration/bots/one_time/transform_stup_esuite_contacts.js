...
module.exports = (opts) => {
  ...
  let bot
  beforeEach(() => {
    bot = opts.getBot('one_time/transform_stup_esuite_contacts')
  })

  describe('#tranform one time esuite file payload to Vibe', async function () {
    beforeEach(async () => {
      await Promise.all([
    ...
    it('should transform imported contacts', async () => {
      // NOTE!!! Not found rows get process last per design
      opts.bus.inQueueData = [
        ...
        ['40001000|#|Sally|#|Stamper|#|09 817 1361|#||#|stamper@stamper.com|#|41 Huia Rd|#||#|Auckland|#|Titirangi|#||#|4555|#|NZ|#|N|#|N|#|N|#|1|#|N|#|N|#||#|1989|#|12|#|8|#|2009|#|11|#|1|#|customerManager|#|162489']
      ] // inQueueData
      ...
      opts.bus.inQueueData.forEach(u => {
      ...
      const ex1 = {
        ...
        customer_person_id_esuite: dataForPersonIDs[1],
      ...
      const ex2 = ['40001000|#|Sally|#|Stamper|#|09 817 1361|#||#|stamper@stamper.com|#|41 Huia Rd|#||#|Auckland|#|Titirangi|#||#|4555|#|NZ|#|N|#|N|#|N|#|1|#|N|#|N|#||#|1989|#|12|#|8|#|2009|#|11|#|1|#|customerManager|#|162489']
      ...
      ex0.updated_at = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss')
      out0.updated_at = ex0.updated_at
      ex1.updated_at = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss')
      out1.updated_at = ex1.updated_at

      assert.deepEqual(out0, ex0)
      assert.deepEqual(out1, ex1)
      assert.deepEqual(out2, ex2)
    })

    it.skip('should add contact_id for existing contacts', async () => {
      const in0 = ['4000547|#|Sally|#|👠Stamper|#|09 817 1361|#||#|stamper@stamper.com|#|41 Huia Rd|#||#|Auckland|#|Titirangi|#||#|4555|#|NZ|#|N|#|N|#|N|#|1|#|N|#|N|#||#|1989|#|12|#|8|#|2009|#|11|#|1|#|customerManager|#|16248']
      const in1 = ['4000547|#|a|#|a|#||#||#|icentris.qa15@gmail.com|#||#||#||#||#||#||#||#||#||#||#||#||#||#||#|0|#|0|#|0|#|0|#|0|#|0|#|showGuest|#|']
      ...
      await opts.mysql.execute('INSERT INTO users (consultant_id) VALUES (4000547)')
      await opts.mysql.execute(`INSERT INTO pyr_contacts (user_id, customer_person_id_esuite) VALUES (1, '${dataForPersonIDs[0]}')`)
      await opts.mysql.execute(`
        INSERT INTO pyr_contacts (user_id, customer_person_id_esuite) VALUES (1, '${dataForPersonIDs[1]}')`)
      opts.event = {
        icentris_client: 'bluesun',
        source: 'stup-new-esuite-contacts',
        destination: 'stup-transformed-esuite-contacts'
      }
      await bot.handle(opts.event, opts.context)
      const ex0 = {
        icentris_client: 'bluesun',
      ...
      ex0.updated_at = updatedAt
      out0.updated_at = updatedAt
      ex1.updated_at = updatedAt
      out1.updated_at = updatedAt
