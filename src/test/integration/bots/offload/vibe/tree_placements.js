...
{"id": "3", "parentId": "1", "lft": "2", "rgt": "3", "position": "0", "level": "2"}`
  ...
  let bot
  beforeEach(async () => {
    bot = opts.getBot('offload/vibe/tree/placements')

    const path = `${process.cwd()}/test/mocks/icentris-bus-${process.env.NODE_ENV}/tree/placement`
    fs.writeFileSync(`${path}/bluesun.ndjson`, data)

    opts.mysql.truncate('tree_placements')
  ...
  describe('#handle', () => {
    it('creates the tmp placements tbl, reads the s3 file contents and writes them to the tmp tbl in vibe', async () => {
      try {
